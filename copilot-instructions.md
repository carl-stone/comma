...
**Regenerate documentation** (always do this after editing roxygen
comments in `R/*.R`):
...
**Run tests** (two options — `devtools` may not be available in all
environments):
...
**Full R CMD check** (CI runs this — must pass with 0 errors, 0
warnings, 0 notes):
...
`pkgload`/`testthat` fallback above. Do not install packages outside
`renv`.
...
1.  **R-CMD-check.yaml** — `R CMD check --no-manual --as-cran` on R 4.5
    / Ubuntu. This runs with `--run-donttest`, so `\donttest{}` examples
    ARE executed. Use `\dontrun{}` for examples that need external
    files.
2.  **pkgdown.yaml** — builds the pkgdown site. Deploys to gh-pages on
    main pushes only.
...
**Naming:** - S4 class: camelCase lowercase-first (`commaData`) -
Analysis functions: verbNoun camelCase (`annotateSites`, `diffMethyl`) -
Plot functions: `plot_noun()` snake_case (`plot_volcano`,
`plot_metagene`) - Internal functions: `.` prefix (`.parseBetaValues`) -
Arguments: snake_case (`mod_type`, `min_coverage`) - Test files:
`test-functionName.R`

**Hard rules:** - Every exported function takes `commaData` as primary
input - Use
...
[`dplyr::rename()`](https://dplyr.tidyverse.org/reference/rename.html)
explicitly -
[`matrixStats::count()`](https://rdrr.io/pkg/matrixStats/man/rowCounts.html)
masks
...
6.  Ensure `NAMESPACE` and `man/` are regenerated (they are
