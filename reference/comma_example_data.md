# Synthetic example methylation dataset for the commaKit package

A
[`commaData`](https://carl-stone.github.io/commaKit/reference/commaData.md)
object built from simulated Oxford Nanopore methylation data. This
dataset is used in package examples, vignettes, and tests. It does not
represent real biological data.

## Format

A `commaData` object with:

- **Sites**:

  588 methylation sites (393 6mA GATC sites and 195 5mC CCWGG sites) on
  a simulated 100 kb chromosome (`"chr_sim"`).

- **Samples**:

  6 samples: `ctrl_1`, `ctrl_2`, `ctrl_3` (condition `"control"`) and
  `treat_1`, `treat_2`, `treat_3` (condition `"treatment"`).

- **Assays**:

  `methylation`

  :   Beta values (proportion methylated reads, 0–1). `NA` where
      coverage \< 5.

  `coverage`

  :   Integer read depth (10–150 per site).

  `mod_counts`

  :   Integer modified-read counts reconstructed from the simulated beta
      and coverage values.

  `canonical_counts`

  :   Integer canonical-read counts reconstructed as
      `coverage - mod_counts`.

  `other_mod_counts`

  :   Not present in this legacy simulated dataset; newly constructed
      modkit objects include this assay when `Nother_mod` is available.

- **rowRanges**:

  GRanges with 1-bp ranges per site. mcols include `mod_type`, `motif`,
  `mod_context`, and `is_diff` (ground-truth differential methylation
  status for testing).

- **colData**:

  Per-sample metadata: `sample_name`, `condition`, `replicate`,
  `caller`.

- **Seqinfo**:

  Chromosome sizes: `c(chr_sim = 100000L)`.

- **annotation**:

  5 simulated genomic features (genes, rRNA, tRNA).

- **motifSites**:

  Empty `GRanges` (no motif search was run).

## Source

Simulated data generated by `data-raw/create_example_data.R`

## Value

A
[`commaData`](https://carl-stone.github.io/commaKit/reference/commaData.md)
object (see Details and Format sections above for structure).

## Details

The dataset simulates an experiment where 30 of the 393 6mA GATC sites
are differentially methylated between control and treatment conditions.
Control samples have high methylation at these sites (~0.9 beta), while
the treatment samples have low methylation (~0.25 beta). This ground
truth is stored in `rowData(comma_example_data)$is_diff` for use in
testing
[`diffMethyl()`](https://carl-stone.github.io/commaKit/reference/diffMethyl.md)
(implemented in Phase 4).

5mC CCWGG sites are not differentially methylated and serve to test
multi- modification-type functionality.

The dataset was generated by `data-raw/create_example_data.R` using
`set.seed(1312)`.

## Examples

``` r
data(comma_example_data)
comma_example_data
#> class: commaData
#> sites: 588 | samples: 6
#> mod types: 5mC, 6mA
#> motifs: CCWGG, GATC
#> mod contexts: 5mC_CCWGG, 6mA_GATC
#> conditions: control, treatment
#> genome: 1 chromosome (100,000 bp total)
#> annotation: 5 features
#> motif sites: none
#> caller: modkit
#> min_coverage: 5
modTypes(comma_example_data)
#> [1] "5mC" "6mA"
motifs(comma_example_data)
#> [1] "CCWGG" "GATC" 
sampleInfo(comma_example_data)
#>         sample_name condition replicate caller
#> ctrl_1       ctrl_1   control         1 modkit
#> ctrl_2       ctrl_2   control         2 modkit
#> ctrl_3       ctrl_3   control         3 modkit
#> treat_1     treat_1 treatment         1 modkit
#> treat_2     treat_2 treatment         2 modkit
#> treat_3     treat_3 treatment         3 modkit
dim(methylation(comma_example_data))
#> [1] 588   6
```
